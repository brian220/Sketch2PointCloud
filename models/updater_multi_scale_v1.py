...
class PointNet2(nn.Module):
    ...
    Input:
        ...
        xyz_features: xyz projection features [B, N, D]
    ...
    def __init__(self, cfg):
        ...
        self.sa1 = PointNetSetAbstraction(npoint=1024, radius=0.1, nsample=64, in_channel=960 + 3, mlp=[960, 512, 256], group_all=False)
        self.sa2 = PointNetSetAbstraction(npoint=384, radius=0.2, nsample=64, in_channel=256 + 3, mlp=[256, 256, 512], group_all=False)
        self.sa3 = PointNetSetAbstraction(npoint=128, radius=0.4, nsample=64, in_channel=512 + 3, mlp=[512, 512, 1024], group_all=False)
        self.sa4 = PointNetSetAbstraction(npoint=None, radius=None, nsample=None, in_channel=1024 + 3, mlp=[1024, 1024, 1280], group_all=True)
        
        self.fp4 = PointNetFeaturePropagation(in_channel=1024 + 1280, mlp=[1024, 1024])
        self.fp3 = PointNetFeaturePropagation(in_channel=512 + 1024 , mlp=[1024, 512])
        self.fp2 = PointNetFeaturePropagation(in_channel=256 + 512 , mlp=[512, 256])
        self.fp1 = PointNetFeaturePropagation(in_channel=960 + 256 , mlp=[960, 512, 256])
    
    def forward(self, xyz, xyz_features):
        xyz = xyz.transpose(2, 1) # [B, 3, N]
        xyz_features = xyz_features.transpose(2, 1) # [B, D, N]
        
        print('xyz size', xyz.size())
        print('proj feature size', xyz_features.size())

        l0_xyz = xyz
        l0_points = torch.cat((xyz, xyz_features), 1) # [B, 3+D, N]
...
class DisplacementNet(nn.Module):
    ...
    def __init__(self, cfg):
        ...
        self.conv1 = nn.Conv1d(280, 128, 1)
    ...
    def forward(self, pc_features, noises):
        noises = noises.transpose(2, 1) #[B, n_length, N]
        noises = utils.network_utils.var_or_cuda(noises)

        refine_features = torch.cat((pc_features, noises), 1) # concat the img features after each point features
...
class Updater(nn.Module):
    ...
    def forward(self, img, xyz, view_az, view_el):
        ...
        pc_features = self.pointnet2(transform_xyz, proj_features)
        ...
        displacements = self.displacement_net(pc_features, noises)
